...
% clc;
%% Specify folder
surfix = './experiment/experiment_0426/2128/';      % Change this line to match the date and time
...
filename = strcat(surfix,'p2.txt');
p_measure = read_p_measure(filename);
% p_measure = p_measure(:,1:32);
filename = strcat(surfix,'ts_qs2.txt');
[~, Ts] = read_ts_qs(filename);
% Ts = Ts(:,:,1:32);
%% Initial Guess
