...
edge_length = options.dis_holes * (options.n_holes_each_line + 1);
...
T_cubes = gen_cube_location(options.n_cubes * options.measure_times, angle_apart, z_cubes, x_cubes);
cubes = [];
for i = 1:options.n_cubes * options.measure_times
    cube = standard_cubic_workpiece(T_cubes(:,:,i), cube_type(i), options.n_holes_each_line, options.dis_holes, edge_length);
    cubes = [cubes, cube];
end
...
while 1
    tic;
    [error, delta_poe] = multi_kinematic_calibration_poe(robot_poe, qs, p_measures, x_true, type, n_holes_cube, options.n_cubes * options.measure_times);
