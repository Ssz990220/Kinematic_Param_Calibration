function [resnorm, robot_poe] = multi_ball_kinematic_calibration_poe_lsq(robot_poe, qs, p_measure, type, n_balls, n_measures_ball)
...
function [F, Jacob] = func_x(x,n_measures_ball,n_balls,qs,p_measure,type, tmp_poe)
...
if nargout > 1   % Two output arguments
    if type == 2
        J = zeros(3,6*tmp_poe.n_dof + 6,n_points);
    elseif type == 1
        J = zeros(3,6*tmp_poe.n_dof,n_points);
    elseif type == 3
        J = zeros(3,7*tmp_poe.n_dof,n_points);
    end
    for i = 1:n_points
        J_full = tmp_poe.get_J(qs(i,:), type);
        J(:,:,i) = [-skew(x_measure(:,i)),eye(3)]*J_full;
    end
    if type == 1
        G = zeros(n_measures_ball*(n_measures_ball-1)*n_balls/2,6*tmp_poe.n_dof);
    elseif type == 2
        G = zeros(n_measures_ball*(n_measures_ball-1)*n_balls/2,6*tmp_poe.n_dof + 6);
    elseif type == 3
        G = zeros(n_measures_ball*(n_measures_ball-1)*n_balls/2,7*tmp_poe.n_dof);
    end
    counter = 1;
    for m = 1:n_balls
        for i = 1:n_measures_ball
